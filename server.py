...
def loop_handle_connection(server_fd, osc_connection):
    while True:
        handle_connection(server_fd, osc_connection)

def handle_connection(server_fd, osc_connection):
    # Read incoming data
    read_socket, _, _ = select.select([server_fd] + connections, [], [], 0.5)  # Added a timeout for select()
    ...
    for sock in read_socket:
        if sock == server_fd:
            ...
            try:
                data = sock.recv(1024)  # Receive buffer size
                if data:
                ...
                sock.close()
                print(f"Connection closed. Connections left: {len(connections)}")

    # Clean up any closed connections (optional but useful for unplugged Arduinos)
    for conn in connections[:]:
        try:
            conn.send(b'')  # Send an empty byte to check if connection is alive
        except:
            connections.remove(conn)
            conn.close()
            print(f"Stale connection removed. Connections left: {len(connections)}")
